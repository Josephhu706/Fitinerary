    ...
    <div class="max-w-screen-sm mx-auto px-4 py-10">
        ...
        <form @submit.prevent="register" class="p-8 flex flex-col bg-light-grey rounded-md shadow-lg">
            ...
            <div class="flex flex-col mb-2">
             ...
             <div class="flex flex-col mb-2">
                <label for="confirmPassword" class="mb-1 text-sm text-indigo-600">Confirm Password</label>
                <input type="password" required class="p-2 text-gray-500 focus:outline-none" id="confirmPassword" v-model="confirmPassword">
            </div>
            <button type="submit" class="mt-6 py-2 px-6 rounded-sm self-start text-sm text-white bg-indigo-600 duration-200 border-solid border-2 border-transparent hover:border-indigo-600 hover:bg-white hover:text-indigo-600">Register</button>
            
            <router-link class="text-sm mt-6 text-center" :to="{name: 'loginForm'}">
            Already Have An Account? <span class="text-indigo-600">Login</span>
            </router-link>
        </form>
    </div>
</template>
